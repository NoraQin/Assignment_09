...
class FileIO:
    ...
    def save_inventory(file_name: list, lst_Inventory: list) -> None:
        ...
        try:
            with open(file_name[0], 'w') as cd_file, open(file_name[1], 'w') as track_file:
                for disc in lst_Inventory:
                    cd_file.write(disc.get_record())
                    for track in disc.cd_tracks:
                        if track == None:
                            continue
                        track_file.write("{},{}".format(str(disc.cd_id), track.get_record()))
    ...
    def load_inventory(file_name: list) -> list:
        ...
        lst_Inventory = []
        try:
            with open(file_name[0], 'r') as cd_file:
                for cd_line in cd_file:
                    cd_data = cd_line.strip().split(',')
                    cd_row = DC.CD(cd_data[0], cd_data[1], cd_data[2])
                    with open(file_name[1], 'r') as track_file:
                        for track_line in track_file:
                            track_data = track_line.strip().split(',')
                            if int(track_data[0]) == cd_row.cd_id:
                                track_row = DC.Track(track_data[1], track_data[2], track_data[3])
                                cd_row.add_track(track_row)
                    lst_Inventory.append(cd_row)
        except Exception as e:
            print('There was a general error!', e, e.__doc__, type(e), sep='\n')
